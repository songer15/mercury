 ...
 * 2019/9/26 11:24
 */
public class SearchElasticSearchSink_T implements ElasticsearchSinkFunction<Row>, Serializable {
    private String index;

    public SearchElasticSearchSink_T(String index) {
        super();
        this.index = index;
    }
    ...
    private IndexRequest createIndexRequest(Row element) {
        ...
        json.put("searchMsg", element.getField(3));
        json.put("device", element.getField(4));
        json.put("language", element.getField(5));
        json.put("vendorId", element.getField(6));
        json.put("LocalCreateTime", new Date());
        json.put("aggregationType", "SearchAction_Count");
