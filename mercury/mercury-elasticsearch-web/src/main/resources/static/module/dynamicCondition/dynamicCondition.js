...
layui.extend({
    ...
    var createInstance = function(_config){
    	var instance = {
    		...
    		,conditionFieldWidth:_config.conditionFieldWidth || 150 //字段列默认宽度
...
//				  form.render(null, 'conditionDiv'+instance.conditionContainerId);
			  ...
			  if(curEditor){
				  if(conditionOptionVal == "between"){
					  ...
					  conditionValueJq.append("<span style='margin:auto 3px;'>to</span>");
...
//    				conditionHtml += fieldText + blankStr + OptionText + blankStr + rsValueText;
    	...
    	instance.buildConditionHtmlForUnpopup = function(){
    		if(instance.config.conditionTextId){
        		...
        		btnDivJq.append($("<a id='xpl-unpopup-query"+instance.conditionContainerId+"' class='layui-btn xpl-unpopup-query'))>Query</a>"));
        		var hideFlag = instance.config.unpopupShowAddBtn ? "" : " xpl-hide ";
        		btnDivJq.append($("<a class='layui-btn xpl-unpopup-add" + hideFlag + "'))>Add Condition</a>"));
        		if(instance.config.extendBtns){
...
//    				  layer.msg('校验失败');
    ...
    var defaultValueConfig = {
    		...
    		opsMap: {"equal":"equal","like":"like","between":"between"
		         ,"start":"start","end":"end","unequal":"not equal","empty":"isEmpty","notempty":"Not Empty"}, //操作选项列表
    ...
    var dynamicCondition = {
    	...
    	,create:function(config){
    		...
    		instance.logicalOperatorHtml +="	<option value='and'  selected='selected'>AND</option>";
    		instance.logicalOperatorHtml +="	<option value='or'>OR</option>";
